...
FOCUS = {
    ...
    "naval": ("boat_loss_pct",),
    # ⚠ **하한이 걸리는지 재려면 축 하나만 밀어야 한다.** `cost` 는 비용 축
    # **넷**을 고루 우선해서, 24,000 tick 에 카드를 8장 받아도 산악병 축에는
    # 3~4장만 간다 — 가장 깊었던 판이 배율 0.220 으로 하한(0.2) **코앞에서**
    # 멈췄다(2026-09-06 실측). 판 길이를 늘려도 안 되고 폭을 좁혀야 한다.
    "highland": ("cost_highland_pct",),
}
