...
// --- 숫자 표시 (`renderNumber` / `renderTroops`) -----------------------------
//
// ⚠ 이식 누락 백스물여덟. 우리는 병력을 **날것 그대로** 찍고 있었고 원본은
// **10으로 나눈다.** 구간마다 자릿수가 다른 표라 눈으로 옮기면 경계에서 틀린다 —
// 그래서 원본을 실제로 불러 **경계값**을 뽑는다.
{
  const U = await load("src/client/Utils.ts");
  const probes = [
    0, 1, 5, 999, 1000, 1001, 1234, 9999, 10_000, 10_001, 12_345, 99_999,
    100_000, 123_456, 999_999, 1_000_000, 1_234_567, 9_999_999, 10_000_000,
    12_345_678, 999_999_999, 1_000_000_000, 1_234_567_890, 9_999_999_999,
    10_000_000_000, 12_345_678_901, -5,
  ];
  out.render_number = probes.map((n) => ({ n, s: U.renderNumber(n) }));
  out.render_troops = probes.map((n) => ({ n, s: U.renderTroops(n) }));
}
