...
⚠ **판당 20분이다**(9,000 tick 완주, 원본 크기 `map` + 나라 72 + 봇 400).
v0.1 은 0.5초였다 — 지도가 1,600칸에서 **651,569칸**이 됐고 규칙이 전부 들어왔다.
그래서 판을 프로세스로 나눈다. 판끼리 완전히 독립이라 그대로 쪼개진다.
**병렬 3판이 단독 대비 1.2배밖에 안 느리다**(실측) — 코어가 남으니 `--jobs` 를 키운다.

판당 노이즈가 크다. §5.46 에서 **한 판만 보고 규칙 버그로 오판할 뻔했다** —
같은 코드가 seed 에 따라 핵 0발과 4발을 낸다. 최소 3~4판을 보고,
채택 판단은 그보다 많이 본다.
"""
