...
def capture(seed: int, players: int, at: list[float], out: Path,
    ...
    def shoot(when: float) -> None:
        labels = {p.pid: f"{p.name} {st.share(p.pid) * 100:.0f}%"
                  for p in st.alive if st.tiles(p.pid) >= 8}
        top = max(st.players.values(), key=lambda p: st.tiles(p.pid))
        title = (f"seed {seed} · {when:.0f}초 · 1위 {top.name} "
                 f"{st.share(top.pid) * 100:.0f}% · 증강 "
                 + ", ".join(f"{k}{v}" for k, v in sorted(top.augments.items())))
        img = render(st.gmap, tile=tile, seed=seed, labels=labels, title=title)
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--tile", type=int, default=14, help="타일 한 변의 픽셀")
