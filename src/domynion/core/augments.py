...
AUGMENTS: list[Augment] = [
    ...
    Augment("landing", "상륙전", "상륙 퇴각 손실 -30%",
            "boat_loss_pct", -0.30),
    # ⚠ **같은 축에 두 번째 카드가 실린 유일한 자리다**(2026-09-06, §5.127).
    # 열 장이 축 열 개에 하나씩이던 동안 *"같은 축이면 더한다"* 는 위 원칙이
    # **한 번도 안 돌았고**(§5.120), 하한 `max(0.2, …)` 도 카드로는 닿을 수
    # 없는 값이라 잠들어 있었다(§5.117). 이 카드가 둘 다 깨운다 —
    # 산악병 Lv3 + 공병대 Lv3 이면 합이 −1.196 이라 배율이 **음수**가 되고,
    # 하한이 없으면 정복할수록 병력이 **늘어난다.**
    Augment("sappers", "공병대", "구릉·산악 정복 비용 -20%",
            "cost_highland_pct", -0.20),
]
...
class Modifiers:
    ...
    ⚠ 이 규칙이 실제로 도는 축은 `cost_highland_pct` **하나뿐**이다(산악병 +
    공병대). 나머지 아홉은 여전히 카드가 하나씩이라 합산 경로를 안 탄다."""
