...
# 첫 정지와 그 뒤 주기.
#
# ⚠ **첫 정지는 사람의 수명보다 빨라야 한다.** 처음에 3분으로 뒀다가 실제로
# 돌려 보니 **드래프트가 한 번도 안 열렸다** — 방치된 사람이 53~98초에
# 탈락한다(2026-09-04 실측, 3 seed). §7.6 의 *"사람을 방치했더니 155초에
# 탈락하는 걸 보고"* 와 같은 자리다. `docs/design.md` 원안 75초가 맞았다.
#
# 이후 주기 5분은 판 길이(33~37분, §5.111)에서 나온다:
# (35분 − 75초) / 5분 ≈ **일곱 번** — design.md 의 "한 판에 7회" 와 맞는다.
AUGMENT_FIRST_TICK = 75 * TICK_HZ           # 75초
