...
**실시간 영토 확장 전략 게임 — 그 위에 증강형 테크트리를 얹는다.**

<sub><a href="https://openfront.io">openfront.io</a> 에서 모티브를 얻어 제작했다.</sub>
...
증강 계층이 이 게임의 고유한 층이다. 설계는 [`docs/design.md`](docs/design.md).

## 상태

**플레이할 수 있다.** 영토 확장 · 골드 · 건물 · 동맹/배신 · 상륙 · 무역 · 전함 ·
철도 · 핵/낙진 · 둠스데이 클락 · AI 봇이 함께 돌고, PyQt6 UI 로 직접 조작한다.
테스트 **1,219개**.

| | |
|---|---|
| ✅ | 핵심 규칙(P1~P6) · AI 봇 · 스폰 · **UI** |
| ✅ | **증강형 테크트리** — 카드 10종 · 드래프트 창 · 보유 표시. 켜면 생존판 영토가 **2.9배**(A/B 24판) |
| ⏳ | Overtime(교착 방지)을 켰다 — 기준선을 다시 재는 중 |

**만들면서 무엇이 어떻게 깨졌는지는 [개발 노트 7편](docs/notes/)에 적었다** — A/B가
아예 다른 판을 재고 있었던 것, 끝날 수 없는 판, 한 번도 발동한 적 없는 규칙 등.
...
# 해상도: map16x(1/16) · map4x(기본, 1000×500) · map(전체 크기 2000×1000, 육지 65만)
...
건설 메뉴는 **건설/업그레이드 통합**이다. 같은 종류가 15칸 안에 이미
...
src/domynion/
  ...
  ui/
    ...
    status.py      이름 옆 상태 깃발 — Qt 없는 순수 계산
    overlays.py    핵 낙하 예고 원 · 상륙 고리 · 국경 관계 · 파도 문구
                   — 같은 방침 (Qt 없는 순수 계산)
    rates.py       병력 `+N/s` · 골드 `+N` — 같은 방침
